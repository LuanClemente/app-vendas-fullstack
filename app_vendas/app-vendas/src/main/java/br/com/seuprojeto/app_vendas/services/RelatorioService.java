...
public class RelatorioService {
    ...
    private final MetaClienteService metaClienteService; // Nova dependência

    @Autowired
    public RelatorioService(VendaRepository vendaRepository, MetaRepository metaRepository, MetaClienteService metaClienteService) {
        this.vendaRepository = vendaRepository;
        this.metaRepository = metaRepository;
        this.metaClienteService = metaClienteService; // Injeção da nova dependência
    }
    ...
    public DashboardVendedorDTO getDashboardVendedor(Usuario vendedor) {
        ...
        BigDecimal totalVendido = vendasDoMes.stream()
                                            .map(Venda::getValor)
                                            .reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal totalCortes = vendasDoMes.stream()
            .filter(venda -> venda.getValor().compareTo(BigDecimal.ZERO) < 0)
            .map(Venda::getValor)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal totalFaturado = vendasDoMes.stream()
            .filter(Venda::isFaturada) 
            .map(Venda::getValor)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        if (valorMeta.compareTo(BigDecimal.ZERO) > 0) {
            porcentagemAtingida = totalVendido.divide(valorMeta, 4, RoundingMode.HALF_UP)
                                              .multiply(new BigDecimal("100")).doubleValue();
        }
        double porcentagemFaturada = 0.0;
        if (valorMeta.compareTo(BigDecimal.ZERO) > 0) {
            porcentagemFaturada = totalFaturado.divide(valorMeta, 4, RoundingMode.HALF_UP)
                                               .multiply(new BigDecimal("100")).doubleValue();
        }
    ...
    public List<ProgressoMetaClienteDTO> getProgressoMetasClientes(Usuario usuario, int mes, int ano) {
        ...
        return metas.stream().map(meta -> {
            ...
            if (valorMeta.compareTo(BigDecimal.ZERO) > 0) {
                porcentagem = totalVendido.divide(valorMeta, 4, RoundingMode.HALF_UP)
                                           .multiply(new BigDecimal("100")).doubleValue();
            }

            return new ProgressoMetaClienteDTO(
                meta.getId(),
                meta.getCliente().getNomeContato(),
                meta.getCliente().getNomeEmpresa(),
                valorMeta,
                totalVendido,
                porcentagem
            );
